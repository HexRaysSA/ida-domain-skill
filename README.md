...
# Bootstrap the API reference documentation
/ida-domain-bootstrap
```

The bootstrap command generates `API_REFERENCE.md` from the installed ida-domain source code, giving Claude comprehensive API documentation.
...
# Bootstrap the API reference documentation (run in Claude Code)
/ida-domain-bootstrap
```
...
3. Navigate to the skill directory and run setup:
   ...
   uv run python setup.py
   ```
4. Bootstrap the API reference documentation by running `/ida-domain-bootstrap` in Claude Code
...
│       └── API_REFERENCE.md     # Generated by /ida-domain-bootstrap
...
Claude will automatically load `API_REFERENCE.md` when needed for comprehensive documentation on functions, strings, cross-references, segments, types, decompilation, and more. This file is generated by running `/ida-domain-bootstrap` after installation, which extracts documentation directly from the ida-domain source code.
